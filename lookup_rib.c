 ...
 */
void
lookup_rib(struct in6_addr *dst_addr, char *if_name)
{
	struct pollfd		 pfd;
	struct timespec	 now, stop, timeout;
	int			 rlen, pid, seq, nfds;

	log_debug("%s: lookup interface of destination %s in routing table",
	    __func__, in6_ntoa(dst_addr));
	...
	while (1) {
		...
		if (rtm->rtm_version == RTM_VERSION && rtm->rtm_seq == seq &&
		    rtm->rtm_pid == pid) {
			log_debug("%s: rtsock read %d bytes: ver %d type %d "
			    "seq %d pid %d flag %#x",
			    __func__, rlen, rtm->rtm_version,
			    rtm->rtm_type, rtm->rtm_seq, rtm->rtm_pid,
			    rtm->rtm_flags);
...
static void
assemble_rtmsg(struct in6_addr *dst_addr, int seq)
{
...
static void
parse_rtmsg(int msglen, char *if_name)
{
	struct sockaddr	*sa;
	...
	if (rtm->rtm_version != RTM_VERSION) {
		log_warning("routing message version %u not understood",
		    rtm->rtm_version);
		return;
	}
		...
		for (int i = 1; i; i <<= 1) {
			if (i & rtm->rtm_addrs) {
				...
				if (i == RTA_IFP && sa->sa_family == AF_LINK) {
					...
					log_debug("%s: lookup routing table "
					    "result: %s",
					    __func__, if_name);
				}
				ADVANCE(cp, sa);
